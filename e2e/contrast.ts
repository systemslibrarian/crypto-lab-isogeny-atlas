 ...
 *    files the node under `incomplete`. The hero's "why it matters" card is a
 *    `color-mix(in oklab, ...)` accent wash over the page rather than a flat
 *    colour, so its text composites onto whatever is behind it.
 *  - text faded by an ancestor's `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. `.cl-hero-sub` carries an
 *    `opacity: .85` from the shared hero styles, overridden back to 1 here
 *    precisely because dimming an already-muted token twice is what breaks it.
 *
 ...
 *
 *  - TEXT SCROLLED OUT OF A CLIPPING ANCESTOR PAINTS NOTHING. `.atlas-scroll`
 *    is an `overflow-x: auto` pan surface around a graph that carries
 *    `min-width: 640px`, so at 380px most of the atlas is clipped away; the
 *    `.mono-dump` holding all 37 adjacency rows and the `.table-scroll` around
 *    the scale table are the same shape. Content scrolled past the client box is
 ...
 *    load-bearing here: the atlas is one `<svg>` holding 37 vertices, each drawn
 *    as a `<circle>`/`<rect>` followed by its `<text>` j-invariant label, so a
 *    label's backdrop is a PRECEDING SIBLING shape, not an ancestor. An
 *    ancestor-only walk would composite those 37 labels onto the svg's own
...
export async function auditContrast(page: Page): Promise<ContrastFailure[]> {
  return page.evaluate(() => {
     ...
     * matters" wash is `color-mix(in oklab, ...)`, and Chromium reports that
     ...
     *
     * This page renders 37 vertices with a label each, a 37-row adjacency dump,
     * and a status line rebuilt on every interaction, and every one of them
     * walks the same handful of ancestors. Without
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. This page's disclosures are the expert layer under each
      // exhibit — the derivation notes, the four live self-checks and the full
      // adjacency dump — and the gate opens each by clicking its summary and
      // scans the revealed state explicitly.
      ...
      // header's `.cl-skip-link` parks at `top: -3rem` and slides into view
      // only on focus. Measuring the parked copy invents a failure
