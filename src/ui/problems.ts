...
export const PROBLEMS: readonly Problem[] = [
  ...
  },
  {
    ...
    boundary:
      '<strong>Model note.</strong> The bit walk is a genuine deterministic walk on the real computed graph, but it is a teaching simplification of the CGL construction: parallel isogenies with the same endpoint are collapsed into one choice, self-loops are skipped, and bits index neighbors in a canonical order rather than selecting kernel subgroups. Non-backtracking is enforced by previous <em>vertex</em>, not by dual isogeny — except where the edge just traversed has multiplicity ≥ 2, in which case several distinct isogenies join the pair, only one is the dual, and returning along another is allowed. On a 37-vertex graph that exception is what keeps the walk moving at all: without it a walk reaching j = 0 or j ≡ 1728 from its only neighbor has no legal move and stops mid-message. The status line reports how many bits were actually consumed and how many steps had only one option. A production CGL hash fixes kernel-level conventions instead; the security story told here is unaffected.',
    precise:
  ...
  },
  {
    ...
    boundary:
      '<strong>Model note.</strong> The collision finder enumerates the same simplified bit walk as Problem 5 (parallel edges collapsed, self-loops skipped, canonical neighbor order, and non-backtracking by previous vertex except across a multiplicity-≥2 edge). The collision it exhibits is real — two genuinely different walks in the genuine graph with the same endpoint, checked to be different paths and not merely different bit-strings — but the walk model is the teaching simplification, not a kernel-labelled CGL transition system.',
    precise:
